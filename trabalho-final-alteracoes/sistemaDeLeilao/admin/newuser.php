...
if(isset($_POST['submit']))
{
  ...
  if ($LastInsertId>0) {
    echo '<script>alert("O setor foi adicionado.")</script>';
    echo "<script>window.location.href ='newsector.php'</script>";
    }
  else
    {
     echo '<script>alert("Alguma coisa está errada. Por favor tenta de novo.")</script>';
    }
}
?>
<!DOCTYPE html>
          ...
          <div class="content-wrapper">
            <div class="row">
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Formulário Adicionar Setor </h4>
                    <form class="forms-sample" method="post">
                      <div class="form-group">
                        <label for="exampleInputName1">Nome do sector</label>
                        <input type="text" name="sectorname" class="form-control" id="sectorname" placeholder="Sector Name" required>
                      </div>
                      <div class="form-group">
                        <label for="exampleTextarea1">Descrição do Sector </label>
                        <textarea class="form-control" name="sectordes" id="sectordes" rows="4" required></textarea>
                      </div>
                      <button type="submit" name="submit" class="btn btn-primary btn-fw mr-2">enviar</button>
                    </form>
                  </div>
                </div>
              </div>
